...
class StyleProvidersTest {
    ...
    public void stripedRowsProviderAlternatesRowColors() {
        StyleProvider styleProvider = StyleProviders.stripedRows(new CellColor(1, 1, 1), new CellColor(2, 2, 2));

        TableWritingContext tableWritingContext = new TableWritingContext();
        tableWritingContext.setWritingTable(true);
        WritingContext writingContext = new WritingContext(tableWritingContext, new CellWritingContext());

        tableWritingContext.setCurrentTableRow(0);
        assertEquals(new CellColor(1, 1, 1), styleProvider.getStyle(writingContext, "even row").getBackgroundColor());

        tableWritingContext.setCurrentTableRow(1);
        assertEquals(new CellColor(2, 2, 2), styleProvider.getStyle(writingContext, "odd row").getBackgroundColor());
    }
}
